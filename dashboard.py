    ...
    # ==============================================================================
        ...
        if st.button("Generar Reporte Completo con IA y Gráficas"): # Instancia el botón de ejecución principal para iniciar la compilación del reporte interactivo
            with st.spinner("Analizando métricas, dibujando gráficos y estructurando su PDF..."): # Activa una animación de carga (spinner) mientras se procesan los datos de fondo
                ...
                # ------------------------------------------------------------------
                # CONFIGURACIÓN DE LOS PROMPTS PARA EL MODELO DE IA
                # ------------------------------------------------------------------

                prompt_sistema = (
                    "Eres un consultor experto en analítica de datos médicos y gestión "
                    "de riesgos de salud. Genera reportes ejecutivos limpios, "
                    "profesionales y empáticos para el paciente."
                ) # Define el rol y comportamiento que tendrá la IA durante la generación del análisis
                ...
                - Gasto Médico Máximo: ${gasto_maximo:,.2f}
                - Edad Promedio: {edad_promedio:.1f} años
                - IMC Promedio: {imc_promedio:.1f}
                - Porcentaje de Fumadores: {porcentaje_fumadores:.1f}%
                - Pacientes Críticos (Obeso + Fumador): {pacientes_alto_riesgo}

                Escribe directamente dos párrafos breves con recomendaciones preventivas.
                """ # Construye dinámicamente el mensaje que recibirá la IA utilizando las métricas calculadas previamente

                # ------------------------------------------------------------------
                # LLAMADA SEGURA A GROQ CON VALIDACIÓN DE SECRETOS
                # ------------------------------------------------------------------

                try: # Inicia un bloque de control para capturar cualquier error relacionado con la API

                    if "GROQ_API_KEY" in st.secrets: # Verifica si la clave API fue configurada correctamente en Streamlit Secrets

                        client = Groq(
                            api_key=st.secrets["GROQ_API_KEY"]
                        ) # Inicializa el cliente oficial de Groq utilizando la clave privada almacenada de forma segura
                        ...
                        chat_completion = client.chat.completions.create(
                            messages=[
                                ...
                                }, # Envía las instrucciones generales que definen el comportamiento del modelo
                                {
                                    "role": "user",
                                    "content": prompt_usuario
                                } # Envía las métricas reales calculadas para generar el análisis ejecutivo
                            ],
                            model="llama-3.3-70b-versatile", # Selecciona el modelo Llama de alto desempeño disponible en Groq
                            temperature=0.3, # Reduce la creatividad para obtener respuestas más estables y técnicas
                            max_tokens=400 # Limita la longitud máxima de la respuesta generada
                        )

                        texto_ia = chat_completion.choices[0].message.content
                        # Extrae el contenido textual generado por la IA y lo almacena para usarlo posteriormente en el PDF

                    else: # Se ejecuta si la variable GROQ_API_KEY no existe dentro de Streamlit Secrets

                        texto_ia = f"""
                        El análisis poblacional evidencia un gasto médico promedio de
                        ${gasto_promedio:,.2f}, con una edad promedio de
                        {edad_promedio:.1f} años.

                        El porcentaje de fumadores alcanza el
                        {porcentaje_fumadores:.1f}% y se identificaron
                        {pacientes_alto_riesgo} pacientes clasificados como de alto riesgo
                        debido a la combinación de obesidad y tabaquismo.

                        Desde una perspectiva preventiva, se recomienda fortalecer
                        programas de control de peso, actividad física regular y
                        reducción del consumo de tabaco para disminuir la probabilidad
                        de enfermedades crónicas y reducir los costos futuros de atención médica.
                        """
                        # Genera automáticamente un análisis alternativo para evitar que falle la creación del reporte

                except Exception as e: # Captura cualquier error de conexión, autenticación o límite de uso de la API

                    texto_ia = f"""
                    No fue posible establecer conexión con Groq.

                    Se generó un análisis local de respaldo.

                    Gasto promedio: ${gasto_promedio:,.2f}
                    Edad promedio: {edad_promedio:.1f} años
                    Porcentaje de fumadores: {porcentaje_fumadores:.1f}%

                    Los resultados indican que los principales factores de riesgo
                    continúan siendo el tabaquismo y los niveles elevados de IMC.

                    Se recomienda fortalecer acciones preventivas orientadas a estilos
                    de vida saludables para reducir riesgos clínicos y financieros.
                    """
                    # Asigna un mensaje de contingencia para que el PDF pueda generarse incluso si la API presenta fallos
